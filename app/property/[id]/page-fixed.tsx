...
export default function PropertyDetailPage() {
  ...
  const handleInquiry = async (e: React.FormEvent) => {
    ...
    try {
      // Fallback to mailto since EmailJS requires setup
      const subject = `Inquiry about ${property?.title}`
      const body = `Hi ${property?.contact.name},

I'm interested in your property "${property?.title}" listed for ${formatPrice(property?.price || 0, property?.type || 'sale')} in ${property?.location}.
...
${contactForm.name}`
      ...
      window.open(`mailto:${property?.contact.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`)
      
      toast({
        title: "Email opened",
        description: "Your email client has been opened with the inquiry message.",
      })
